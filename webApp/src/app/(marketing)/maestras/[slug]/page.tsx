...
import { MAESTRA_DATA } from "@/lib/mocks/maestra";
...
function BirthChart() {
  ...
  const { planets } = MAESTRA_DATA.birthChart;
...
export default async function MaestraPage({ params }: { params: Promise<{ slug: string }> }) {
  await params;
  const M = MAESTRA_DATA;

  return (
    <div className="min-h-screen bg-cosmos-0">
      <Breadcrumb crumbs={[{ label:"Maestras", href:"/maestras" }, { label:M.name }]} />
        ...
        <div className="mx-auto max-w-5xl">
          ...
          <div className="grid md:grid-cols-[1.2fr_1fr] gap-10 items-start">
            ...
            <aside className="cosmos-card p-8">
              <span className="block mb-4 text-4xl text-gold-400 font-quote">❝</span>
          ...
          <div className="grid gap-5 md:grid-cols-3">
            {M.testimonios.map((t) => (
              <blockquote key={t.who} className="cosmos-card relative p-7">
                <span className="pointer-events-none absolute left-4 top-1 select-none font-quote text-[64px] leading-none text-lila-300/20">❝</span>
      ...
      <section className="relative overflow-hidden border-t border-lila-300/18 px-6 py-24 md:px-12 text-center"
        ...
        <div className="flex flex-col sm:flex-row gap-3.5 justify-center">
          <a href="/cursos/tarot-iniciatico" className="btn-ritual btn-ritual-primary rounded-pill">Tarot iniciático · 6 mayo ↦</a>
